...
            //Debug.Log(CalculateChromaticAberration(new Color32(255,255,255,255),new Color32(255,255,255,254)));
            canGridList.Clear();
...
            int xLeft = -1;
            ...
            for (int h = 0; h < height; h++) {
                ...
                for (int w = 0; w < width - 1; w++) { //自左向右搜索每一行的可归并像素区间
                    Color32 leftPixel = sourcePixels[h * width + rowLeft]; //h * width + w
                    Color32 rightPixel = sourcePixels[h * width + w + 1];
                    if (CalculateChromaticAberration(leftPixel, rightPixel)) { //色差检测
                        rowRight++;
                    } else {
                        if (rowLeft <= rowRight) {
                            //存在重叠区域
...
                    Color32 topPixel = sourcePixels[(h + 1) * width + w];
                    if (CalculateChromaticAberration(bottomPixel, topPixel)) { //色差检测
                        columnTop++;
                    } else {
...
        enum ColorSpace {
        ...
        static bool CalculateChromaticAberration(Color32 a, Color32 b) {
